...
def render():
    ...
    user_id = st.text_input("Enter WID/UserID", key="lhours_single_user")
    if st.button("Get Learning Hours", key="lhours_single_btn"):
        if user_id.strip():
            resp = get_single(FEATURE_SLUG, user_id.strip())
            if "error" in resp:
                st.error(resp["error"])
            else:
                render_table_with_download([resp], f"learning_hours_{user_id}")
        else:
    ...
    uploaded = excel_uploader("lhours_bulk_upload")
    if st.button("Process Bulk", key="lhours_bulk_btn"):
        if uploaded is None:
            ...
            if "error" in resp:
                ...
                render_table_with_download(resp.get("data", []), "learning_hours_bulk")
